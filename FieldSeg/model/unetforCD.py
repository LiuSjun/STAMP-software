...
def get_segmentation_headCD():
    kernel_size = 3
    in_channels = 16*2
    out_channels = [32, 16, 8, 4, 1,1]
    conv2d = nn.Conv2d(in_channels, out_channels[0], kernel_size=(kernel_size, kernel_size), padding=kernel_size // 2)
    conv2d1 = nn.Conv2d(out_channels[0], out_channels[1], kernel_size=(kernel_size, kernel_size), padding=kernel_size // 2)
    conv2d2 = nn.Conv2d(out_channels[1], out_channels[2], kernel_size=(kernel_size, kernel_size), padding=kernel_size // 2)
    conv2d3 = nn.Conv2d(out_channels[2], out_channels[3], kernel_size=(kernel_size, kernel_size), padding=kernel_size // 2)
    conv2d4 = nn.Conv2d(out_channels[3], out_channels[4], kernel_size=(kernel_size, kernel_size), padding=kernel_size // 2)
    conv2d5 = nn.Conv2d(out_channels[4], out_channels[5], kernel_size=(kernel_size, kernel_size), padding=kernel_size // 2)
    return nn.Sequential(conv2d,conv2d1,conv2d2,conv2d3,conv2d4, conv2d5, nn.Sigmoid())
...
class UNet_Bou(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.encoder = get_encoder(in_channels= 3)  # 修改波段数的时候记得把梯度数也改回来
...
class UNet_BouCD(nn.Module):
    ...
    def __init__(self):
        ...
        # self.encoder2 = get_encoder(in_channels=8)
        self.decoder = get_decoder()
        self.segmentation_head = get_segmentation_headCD()
    ...
    def forward(self, x, y):
        ...
        # cat = torch.cat((x, y), 1)
        # cat = y - x
        featuresx = self.encoder(x)
        featuresy = self.encoder(y)
        # features = [torch.cat((featuresy[i], featuresx[i]), 1) for i in range(0, len(featuresx))]
        decoder_outputx = self.decoder(featuresx)
        decoder_outputy = self.decoder(featuresy)
        decoder_output = torch.cat((decoder_outputx, decoder_outputy), 1)
        masks = self.segmentation_head(decoder_output)
