...
def tranining(dataset_tra, dataset_val, dataset_tst, bestpthname):
    # dataset = DatasetGF2(
    #     (dts_ah.DatasetAH(), dts_cd.DatasetCD(), dts_zj.DatasetZJ(), dts_hlj.DatasetHLJ(), )
    # )
    writer = sw(r'G:\FinalsForData\tensorboard\reuet_bou', filename_suffix='0911')
    ptn_name = "resnet_rgbwithoutHLG_{}.pth"
    batch_size = 10
...
def traniningten():
    datasets = (DatasetAH(), DatasetCD(), DatasetGS(), DatasetMS(), DatasetZJ(), DatasetZZ())
    dataset = DatasetGF2(
        datasets
    )
