...
if __name__ == "__main__":
    ...
    # predict_subimg()
    img_target_path = r"E:\Newsegdataset\崇州\test"    #r"F:\QGIS download\mk0213-2"
    image_list = os.listdir(img_target_path)
    for k in range(len(image_list)):
        if image_list[k][-4:] == '.tif':
            ...
            util_dts.normalize_apply(img_target, util_dts.normalize_parameters(img_target))
            img_target = img_target / 255

            name_ptn = os.path.join(ROOT_PTN, "resnet_seg_0.8982074975967407.pth")   #resnet1227_seg_0.9692617136499156resnet1208_seg_6datasets_0.8858911735670907"resnet1208_seg_6datasets_0.8858911735670907.pth"
            img_seg = predict_image_center(img_target,
                                             BasicUNet(),
                                             name_ptn)
            # img_seg_name = os.path.join(ROOT_RST, "{}_seg.tif".format(image_list[k][:-4]))
            # UNIT.numpy2img(img_seg_name, img_seg, proj=proj, geot=geot)

            name_ptn = os.path.join(ROOT_PTN, "resnet_bou_0.6152148365974426.pth") #resnet_bou_0.6463043689727783resnet1215_bou_0.5788623491923014.pth")  # resnet1208_bou_6datasets_0.56199397785323.pth
            img_bou = predict_image_center(img_target,
                                             BasicUNet(),
                                             name_ptn)
            # img_bou_name = os.path.join(ROOT_RST, "{}_bou.tif".format(image_list[k][:-4]))
            # UNIT.numpy2img(img_bou_name, img_bou, proj=proj, geot=geot)
            del img_target
            ...
            # _, img_segnobianjie = cv2.connectedComponents(img_seg.astype(np.uint8))
            # UNIT.numpy2img(os.path.join(ROOT_RST, "{}_nobianjie_result.tif".format(image_list[k][:-4])), img_segnobianjie, proj=proj, geot=geot)  #
            # del img_segnobianjie   #, rgb

            img_seg[img_bou == 1] = 0
            # UNIT.numpy2img(os.path.join(ROOT_RST, "{}_result111.tif".format(image_list[k][:-4])), img_seg, proj=proj,
            #                geot=geot)
            del img_bou
            img_seg = remove_noise(img_seg.astype(np.uint8))
            _, img_seg = cv2.connectedComponents(img_seg.astype(np.uint8))
            # rgb = mark2rgb(img_seg)
            # kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
            # dilated_img = cv2.dilate(img_seg, kernel, iterations=1)
            UNIT.numpy2img(os.path.join(ROOT_RST, "{}_resultwaikuo.tif".format(image_list[k][:-4])), img_seg, proj=proj, geot=geot)  #
            del img_seg   #, rgb
