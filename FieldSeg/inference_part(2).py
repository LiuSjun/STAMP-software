    ...
    with torch.autograd.set_detect_anomaly(True) and torch.no_grad():
        for i in tqdm(range(xlen_chip)):
            for j in range(ylen_chip):
                ...
                if xloc0 != 0 or yloc0 != 0 or xloc1 != xlen or yloc1 != ylen:
...
if __name__ == "__main__":
    ...
    for k in range(len(image_list)):
        ...
        name_ptn = os.path.join(ROOT_PTN, "resnet_rgb_0.9308635393778483.pth")
        img_label = predict_image_center(img_target,
                                         RGBUNet(),
                                         name_ptn)
        img_output = os.path.join(ROOT_RST, "sub1segrgb.tif")
