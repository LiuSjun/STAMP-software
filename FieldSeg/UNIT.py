...
def raster_clip(input_raster, srs_raster, output_raster, datatype='ENVI', invalid=None):
    ...
    # assert iproj == sproj
    s_proj, s_geot = sdts.GetProjection(), sdts.GetGeoTransform()
    i_geot = idts.GetGeoTransform()
    ...
    x0, y0 = xy2pl(pl2xy((0, 0), sdts.GetGeoTransform()), i_geot)
    x1, y1 = xy2pl(pl2xy((sxsize, sysize), sdts.GetGeoTransform()), i_geot)
    ...
    sx0, sy0 = xy2pl(pl2xy((max(int(x0), 0), max(int(y0), 0)), i_geot), s_geot)
    ...
    numpy2img(output_raster, output_img, proj=s_proj, geot=s_geot, type=datatype)
